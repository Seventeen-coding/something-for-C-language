...
static int _bookRoom(BookState *this_state, ROOM_T *this_room);
static int _unsubscribeRoom(BookState *this_state, ROOM_T *this_room);
static int _checkInRoom(BookState *this_state, ROOM_T *this_room);
static int _checkOutRoom(BookState *this_state, ROOM_T *this_room);
...
BookState *CreateBookState()
{
    ...
    do
    {
        if (inheritState((STATE_T *)bookstate) != 0)
        {
            break;
        }
...
static int _checkInRoom(BookState *this_state, ROOM_T *this_room)
{
    ...
    this_room->state = (STATE_T *)this_room->checkOutRoom;
}
